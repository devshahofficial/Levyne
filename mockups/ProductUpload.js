...
class ProductUpload extends React.Component {
	...
	onUploadPress = () => {
		...
		if (!/^[1-9][0-9]?$|^100$/.test(this.state.ApproxManufacturingDays)) {
			return this.setState({
				showCustomToast: true,
				ToastText: 'Approx. production time should be in number ',
			});
		}
				...
				>
					...
					<View paddingH-20>
						...
						<CstmInput
							...
							onChangeText={(Price) => {
								if (
									...
									this.setState({
										Price,
										TotalPrice: parseFloat(Price - Price * 0.01 * this.state.Discount).toFixed(2),
									});
								}
							}}
						/>
						...
						<CstmInput
							...
							onChangeText={(Discount) => {
								if (
									...
									this.setState({
										Discount,
										TotalPrice: parseFloat(this.state.Price - this.state.Price * 0.01 * Discount).toFixed(2),
									});
								}
							}}
						/>
						...
						<CstmInput
							placeholder='In days'
							placeholderTextColor={Colors.grey50}
							value={this.state.ApproxManufacturingDays}
							keyboardType='number-pad'
							maxLength={2}
							onChangeText={(ApproxManufacturingDays) => {
								this.setState({ ApproxManufacturingDays });
							}}
						/>
...
export default connect(mapsStateToProps)(ProductUpload);
